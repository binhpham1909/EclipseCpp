...
SysTime::SysTime() {
	...
	DBG2F("Time zone name: ", _dst.abbrev);
	// NTP time
	_NtpServerName = "pool.ntp.org";
	DBG2F("Time NTP server: ", _NtpServerName);
...
char* SysTime::monthStr(uint8_t month) {
    strcpy_P(dt_buffer, getProgmem (&MonthStr[month]).full);
    return dt_buffer;
}

char* SysTime::dayStr(uint8_t day) {
    strcpy_P(dt_buffer, getProgmem (&DayStr[day]).full);
    return dt_buffer;
}

char* SysTime::monthShortStr(uint8_t month) {
    strcpy_P(dt_buffer, getProgmem (&MonthStr[month]).min);
    return dt_buffer;
}

char* SysTime::dayShortStr(uint8_t day) {
    strcpy_P(dt_buffer, getProgmem (&DayStr[day]).min);
    return dt_buffer;
}
...
bool SysTime::syncNTP() {
	  ...
	  DBG2F0("GMT time is: ", hour());
	  DBG2F0(" - ", minute());
	  DBG2F(" - ", second());
	  time_t tn = nowLocal();
	  DBG2F0("Local time is: ", hour(tn));
	  DBG2F0(" - ", minute(tn));
	  DBG2F(" - ", second(tn));
	  return true;
}
